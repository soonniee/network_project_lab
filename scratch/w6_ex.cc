...
NS_LOG_COMPONENT_DEFINE ("week6_ex");
...
RxDrop(Ptr<const Packet>p){
    NS_LOG_LOGIC("RxDrop at"<<Simulator::Now().GetSeconds());
}
int 
...
//   uint16_t sinkPortUdp = 9090;
  Address sinkAddressTcp (InetSocketAddress (interfaces.GetAddress (1), sinkPortTcp));
...
//   sinkAppUdp.Stop (Seconds (30.));
  ...
  ApplicationContainer sourceAppTcp = onoffTcp.Install(nodes.Get(0));
  sourceAppTcp.Start (Seconds (1.));
  sourceAppTcp.Stop (Seconds (10.));
...
/* ToDo: Implement UDP application
	...
	// ApplicationContainer sourceAppUdp = onoffUdp.Install(nSrc1);
